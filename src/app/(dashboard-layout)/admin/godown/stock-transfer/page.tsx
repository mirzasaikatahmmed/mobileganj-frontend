...
import { ArrowDownUp, Plus, Search, Eye, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export default function StockTransferPage() {
  const transfers = [
    { id: 'GTB-045', date: '2024-01-20', branch: 'Main Branch', items: 5, quantity: 45, requestedBy: 'Branch Manager', status: 'pending' },
    { id: 'GTB-044', date: '2024-01-19', branch: 'Branch 1', items: 3, quantity: 28, requestedBy: 'Admin', status: 'approved' },
    { id: 'GTB-043', date: '2024-01-18', branch: 'Branch 2', items: 8, quantity: 62, requestedBy: 'Manager', status: 'completed' },
    { id: 'GTB-042', date: '2024-01-17', branch: 'Main Branch', items: 4, quantity: 35, requestedBy: 'Staff', status: 'rejected' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-2">
            <ArrowDownUp className="w-6 h-6 sm:w-7 sm:h-7 text-primary" />
            Stock Transfer [G2B]
          </h1>
          <p className="text-muted-foreground text-sm sm:text-base mt-1">
            Manage godown to branch stock transfers
          </p>
        </div>
        <Link href="/admin/inventory/create-transfer">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-card border rounded-lg p-4">
          ...
          <p className="text-2xl font-bold mt-1 text-green-600">48</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-sm font-semibold">To Branch</th>
            ...
            <tbody>
              {transfers.map((transfer) => (
                <tr key={transfer.id} className="border-b hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm">{transfer.branch}</td>
