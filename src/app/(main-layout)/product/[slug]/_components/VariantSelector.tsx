...
export default function VariantSelector({ variants, onVariantChange }: VariantSelectorProps) {
  ...
  const regionOptions = [...new Set(variants.map((v) => v.region).filter(Boolean))];
    ...
    <div className="space-y-5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {storageOptions.map((storage) => (
              <OptionButton key={storage} label={storage!} isSelected={selectedStorage === storage} onClick={() => setSelectedStorage(storage!)} />
            ))}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {colorOptions.map((color) => (
              <OptionButton key={color} label={color!} isSelected={selectedColor === color} onClick={() => setSelectedColor(color!)} />
            ))}
          </div>
        </div>
      )}
      ...
      {ramOptions.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {ramOptions.map((ram) => (
              <OptionButton key={ram} label={ram!} isSelected={selectedRam === ram} onClick={() => setSelectedRam(ram!)} />
            ))}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {regionOptions.map((region) => (
              <OptionButton key={region} label={region!} isSelected={selectedRegion === region} onClick={() => setSelectedRegion(region!)} />
            ))}
          </div>
        </div>
      )}
      ...
      {(selectedStorage || selectedColor || selectedRam || selectedRegion) && (
        ...
        >
          <p className="text-xs font-medium text-muted-foreground mb-2">Selected Configuration:</p>
          <div className="flex flex-wrap gap-1.5">
            {selectedStorage && <Badge variant="secondary" className="text-xs">{selectedStorage}</Badge>}
            {selectedColor && <Badge variant="secondary" className="text-xs">{selectedColor}</Badge>}
            {selectedRam && <Badge variant="secondary" className="text-xs">{selectedRam}</Badge>}
            {selectedRegion && <Badge variant="secondary" className="text-xs">{selectedRegion}</Badge>}
          </div>
        </motion.div>
      )}
    </div>
